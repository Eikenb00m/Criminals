...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['number'])) {
        if (isset($_POST['color']) && !empty($_POST['color']) && $_POST['color'] != 1) {
            $error[] = 'Je hebt geen nummer opgegeven.';
        } else {
            $error[] = 'Je hebt geen nummer opgegeven.';
        }
    } elseif ($_POST['number'] < 0 || $_POST['number'] > 36) {
        $error[] = 'Nummer wat er is ingegeven kan niet.';
    }

    // Check if gamble money has been entered and if the user has that money
    if (empty($_POST['gambleMoney'])) {
    ...
    } elseif ($_POST['color'] < 0 || $_POST['color'] > 3) {
    ...
    if (count($error) > 0) {
        ...
        // Someone thought it would be fun that $_POST containing 0 is just empty...
        $numberPlayer = ($_POST['color'] == 1) ? 0 : $_POST['number'];

        // User won
        if ($numberWon == $numberPlayer) {
            $stmt = $dbCon->prepare('UPDATE users SET cash = cash + :winnings WHERE id = :id');
            $stmt->execute([
                ':winnings' => (int) $_POST['gambleMoney'] * 36,
                ':id' => $userData['id']
            ]);
            $tpl->assign('success', 'Je hebt de roulette gewonnen je wint 36x je inzet!');
        } else {
            // User lost
            $stmt = $dbCon->prepare('UPDATE users SET cash = cash - :loss WHERE id = :id');
            $stmt->execute([
                ':loss' => (int) $_POST['gambleMoney'],
                ':id' => $userData['id']
            ]);
